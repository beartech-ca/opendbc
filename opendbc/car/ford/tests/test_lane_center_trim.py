"""Unit tests for LaneCenterTrim.

Ported from BluePilot (BluePilotDev/bluepilot, bp-dev, MIT) together with the module they
cover. Converted from unittest to pytest style, which this repository requires; the cases
and their numbers are unchanged. They drive the class directly against a minimal
...
class TestLaneCenterTrim:
  ...
  def test_disabled_no_correction(self):
    ...
    assert (self.trim.correction) == (0.0)
  ...
  def test_zero_offset_centered_no_correction(self):
    ...
    assert abs((self.trim.correction) - (0.0)) < 1e-6
  ...
  def test_added_to_kappa_cmd(self):
    ...
    assert abs((result) - (0.01 + 0.004)) < 1e-4
  ...
  def test_no_lanelines_still_applies_offset(self):
    ...
    assert abs((self.trim.correction) - (0.004)) < 1e-4  # same ceiling as good lanelines
  ...
  def test_confident_lanelines_pull_toward_laneline_center_even_with_zero_offset(self):
    ...
    assert (self.trim.correction) > (0.0)  # pulls right, toward center
  ...
  def test_correction_eventually_converges_despite_rate_limit(self):
    ...
    centered_confident = _good_model(lane_center_y=2.0, model_y=0.0)
    self._run(centered_confident, offset=0.0, gain=1.0, iterations=500)
    assert abs((self.trim.correction) - (0.004)) < 1e-3
